...
export class Slideshow extends Component {
  ...
  #updateControlsVisibility() {
    ...
    const { scroller, previous, next, slideshowControls } = this.refs;

    if (!(scroller instanceof HTMLElement)) return;

    if (slideshowControls instanceof HTMLElement && this.hasAttribute('auto-hide-controls')) {
      slideshowControls.hidden = scroller.scrollWidth <= scroller.offsetWidth;
    }

    if (previous instanceof HTMLButtonElement && next instanceof HTMLButtonElement) {
      const canNavigate = this.slides?.length > 1 && scroller.scrollWidth > scroller.offsetWidth + 1;

      previous.disabled = !canNavigate || (!this.infinite && this.current === 0);
      next.disabled = !canNavigate || (!this.infinite && this.nextIndex >= (this.slides?.length || 0));
    }
  }
